...
def compute_Xbar(K, T, x, note_seq_by_column, active_columns, base_corners):
    ...
    for k in range(K + 1):
        ...
        for i in range(1, len(notes_in_pair)):
            ...
            if ((k - 1) not in active_columns[idx_start] and (k - 1) not in active_columns[idx_end]) or (
                    k not in active_columns[idx_start] and k not in active_columns[idx_end]):
                val *= (1 - cross_coeff[k])
            X_ks[k][idx] = val
            fast_cross[k][idx] = max(0, 0.4 * max(delta, 0.06, 0.75 * x) ** (-2) - 80)
    X_base = np.zeros(len(base_corners))
    for i in range(len(base_corners)):
        X_base[i] = sum(X_ks[k][i] * cross_coeff[k] for k in range(K + 1)) + sum(
            np.sqrt(fast_cross[k][i] * cross_coeff[k] * fast_cross[k + 1][i] * cross_coeff[k + 1]) for k in range(0, K))
...
def compute_Pbar(K, T, x, note_seq, LN_rep, anchor, base_corners):
    stream_booster = lambda delta: 1 + 1.7e-7 * ((7.5 / delta) - 160) * ((7.5 / delta) - 360) ** 2 if 160 < (
                7.5 / delta) < 360 else 1
    ...
    for i in range(len(note_seq) - 1):
        ...
        if delta < 2 * x / 3:
            inc = delta ** (-1) * (0.08 * x ** (-1) * (1 - 24 * x ** (-1) * (delta - x / 2) ** 2)) ** (1 / 4) * max(
                b_val, v)
        else:
...
def compute_Abar(K, T, x, note_seq_by_column, active_columns, delta_ks, A_corners, base_corners):
    ...
    for i in range(len(base_corners)):
        ...
        for j in range(len(cols) - 1):
            ...
            dks[k0][i] = abs(delta_ks[k0][i] - delta_ks[k1][i]) + 0.4 * max(0, max(delta_ks[k0][i],
                                                                                   delta_ks[k1][i]) - 0.11)
